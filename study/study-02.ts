...
let memberUser: number | string = 123;

// Union type 이지만 변수에 number로 할당을 한 경우 string 타입이 될 수 없다.
// error : memberUser = 'kim'

// Array union type
let memberUsers: (number | string )[] = [1,'2',3];
// Object union type
let memberUsersObject: {a: string | number} = { a : 123}
...
let userAges: unknown = 1;
// error : userAges + 1;
// userAges는 숫자 1이 들어갔지만 타입이 number가 아니기 때문에 오류 발생


let userName1: string = 'kim';
let userAge1: undefined = undefined;
let married: boolean = false;
let Sion: unknown = [userName1, userAge1, married];
